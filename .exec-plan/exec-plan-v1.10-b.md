...
> **同进程 harness 设计**：提取 Step 4 代码，在 L481（`Set-Content`）与 L489（`Get-Content $tmpPath`）之间注入：
> ```powershell
...
- 注入点：SKILL L479（foreach 循环结束后）与 L480（if 校验）之间
...
3. 入库：commit 文件范围显式列出——
   ...
   - `.exec-plan/exec-plan-v1.10-b.md`（本执行计划）

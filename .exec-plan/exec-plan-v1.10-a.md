...
| heartbeat 失败 | L476 | 裸 return | 新增 `RUN_STATUS\|failed\|步骤4 heartbeat 失败，整轮终止。` + return |
| result.json 读取失败 | L477 | 无 try/catch（异常冒泡） | 新增 try/catch：lock release + `RUN_STATUS\|failed\|读取 result.json 失败` + return |
| stats/items 完整性失败 | L479 | `Release-LockSafely; return`（裸 return） | 新增 `RUN_STATUS\|failed\|review 程序事实完整性校验失败` — **注意：return 被移除，执行落入后续 try 块** |
| review tmp 写入失败 | L486 | 裸 return | 新增 `RUN_STATUS\|failed\|review 写入失败` + return |
| review JSON 校验失败 | L495 | 裸 return | 新增 `RUN_STATUS\|failed\|review 临时 JSON 校验失败` + return |
| review 原子替换失败 | L505 | 裸 return | 新增 `RUN_STATUS\|failed\|review 原子替换失败` + return |
| Changelog | L765 | 不存在 | 新增 v1.10 变更条目 |

**关键发现**：L479 stats/items 完整性失败路径移除了 `return`，输出 `RUN_STATUS|failed|` 后执行落入后续 `try { $doc|ConvertTo-Json... }` 块（L480）。其他 5 条错误路径（L487/L496/L506/heartbeat/result.json-read）均在 `RUN_STATUS|failed|` 后保留 `return`。此不一致性是本轮 T38 验证重点之一。
...
所有测试通过 `GITHUB_VERSION_MONITOR_BASE` 环境变量实现 fixture 隔离。SKILL-v1.10 每个 step 均支持此变量（SKILL L156/L238/L429/L473/L521）。
...
SKILL 中 API URL 硬编码为 `https://api.github.com`（SKILL L338/L478），无 base 覆盖机制。测试采用 **PowerShell 函数覆盖** 方案：
...
| 7 | T04+T26+T18 | {{ISO8601}} | {{ISO8601}} | {{status}} | 状态机+flag+保留确认 | {{result}} |
...
| 7 | T04 + T26 + T18 — 状态机 + Flag + 状态保留 | lib/step2-mock-harness.ps1, fixture | 多个测试目录 + 证据 | 中 |
...
```powershell
git diff --no-index SKILL-v1.9.md SKILL-v1.10.md > .production-validation-v110-final/v19-v110.diff
```
...
|---|---|---|---|---|
| heartbeat 失败 | L476 | 是 | 是 | 是 |
| result.json 读取失败 | L477 | 无 try/catch | 是（新增） | 是（新增） |
| stats/items 完整性失败 | L479 | 是 | **否（移除）** | 是（新增） |
| review tmp 写入失败 | L487 | 是 | 是 | 是（新增） |
| review JSON 校验失败 | L496 | 是 | 是 | 是（新增） |
| review 原子替换失败 | L506 | 是 | 是 | 是（新增） |

> **L479 关键发现**：stats/items 完整性失败路径在 v1.10 中移除了 `return`。输出 `RUN_STATUS|failed|review 程序事实完整性校验失败，整轮终止。` 后执行落入 L480 `try { $doc|ConvertTo-Json... }` 块。Phase 2 的 T38-stats-items 测试须验证此落入行为是否导致二次输出或异常。
...
**设计原因**：SKILL Step 5 中 `$commitSucceeded`（L617/L623）和 `$lockReleased`（L634/L640）是同进程局部变量。若拆分为两个独立脚本通过 `pwsh -File` 执行，跨进程无法传递变量，导致测试失去区分度。
...
**前置变量**：harness 须预置 `$conclusionText` / `$summaryText` / `$noteText`（SKILL L535-543 占位值），否则 Step 5 md 组装失败。
...
`mock-invoke-restmethod.ps1` 须按场景返回仿真对象，成员形状与 SKILL 状态机访问路径一致（SKILL L338-366 直接核实）：

**成功路径**（`Invoke-RestMethod` 正常返回）：

| 场景 | mock 返回对象 | SKILL 访问路径 |
|---|---|---|
| normal / versionJump | `PSCustomObject` 含 `tag_name`(string) + `published_at`(ISO date) | L339 `$j.tag_name` / `$j.published_at` |
| metadata_incomplete | `PSCustomObject` 含 `tag_name`(string) 但无 `published_at` | L344-346 |
| invalid_response | `PSCustomObject` 含空 `tag_name` 或无 `tag_name` | L347-348 |

**异常路径**（`Invoke-RestMethod` 抛异常）：

| 场景 | mock 异常对象 | SKILL 访问路径 |
|---|---|---|
| 404 (not_found) | `Exception.Response.StatusCode = 404` | L359 |
| 401 (auth_error) | `Exception.Response.StatusCode = 401` | L358 |
| 429 (rate_limited) | `Exception.Response.StatusCode = 429` | L360 |
| 403+remaining=0 (rate_limited) | `StatusCode = 403` + `Headers['X-RateLimit-Remaining'] = '0'` | L361 |
| 403+remaining>0 (forbidden) | `StatusCode = 403` + `Headers['X-RateLimit-Remaining'] = '50'` | L361 |
| 5xx (server_error) | `StatusCode = 500` | L362 |
| 其他 HTTP (http_error) | `StatusCode = 302` | L363 |
| network_error | 异常无 `.Exception.Response`（如 timeout） | L364 else 分支 |
...
- `Exception.Response.StatusCode`：SKILL L352 以 `[int]$_.Exception.Response.StatusCode` 显式转换后做数值比较（L358-363），因此 mock 侧使用 `int`（如 `404`）或 `System.Net.HttpStatusCode` 枚举实例均可兼容；推荐枚举实例以贴近真实响应形状。
- `X-RateLimit-Remaining` 取值：SKILL L361 以字符串比较（`$rl -eq '0'`），mock Headers 返回值**必须为字符串** `'0'` / `'50'`（不得为 int）。
...
**对齐自检**：mock 函数库构造完成后、任何测试执行前，sub-agent 须做一次 "contract → SKILL 提取表达式" 对齐自检：按 SKILL L338-364 逐成员模拟访问，确认每个场景取值路径与 contract 表一致，结果记录于 `lib/mock-contract-selfcheck.txt`（含 PS7 Headers 类型选择记录）。
...
> **harness 逐字性验证**：`step5-t39-harness.ps1` 内联复制了 SKILL Step 5 代码并注入一段 PID 修改逻辑。Phase 11 第 4 项验证须对 harness 做受限 diff——与 SKILL 原文 Step 5 代码块仅允许存在一处注入差异（L632-L633 之间的 PID 修改），其余代码须逐字一致。
...
- [ ] 确认 10 项禁止项检查完成
- [ ] 确认 6 条 Step 4 错误路径逐路径核验表完成（含 L479 return 移除发现）
- [ ] 确认 5 个 step 脚本 + 2 个 harness + 4 个辅助工具全部按输出清单产出
...
**目标**: SKILL L480 `Set-Content -Path $tmpPath -Encoding UTF8` 真实失败。
...
**目标**: SKILL L489-490 `Get-Content $tmpPath -Raw|ConvertFrom-Json` 校验失败。
...
> **同进程 harness 设计**：提取 Step 4 代码，在 L480（`Set-Content`）与 L489（`Get-Content $tmpPath`）之间注入：
> ```powershell
> # T38-B 注入：篡改 tmp 文件内容为非法 JSON
> Set-Content $tmpPath -Value '{invalid json' -Force
> ```
> 注入点在 try/catch 块外，技术干净。
...
**目标**: SKILL L499 `Move-Item $tmpPath $resultPath -Force` 真实失败。
...
> **条件化期望**：heartbeat 失败后直接 `return`（L476），未调用 `Release-LockSafely`。锁状态取决于 Step 1 创建的锁是否仍由当前 PID 持有。审查点须根据实际 stdout 判断，不得假设锁已释放。

#### T38-result-read: result.json 读取失败终态验证

**目标**: SKILL L477 新增的 result.json 读取 try/catch。
...
**目标**: SKILL L479 stats/items 完整性失败路径。**此路径在 v1.10 中移除了 `return`，是本轮关键发现。**
...
- 注入点：SKILL L478（foreach 循环结束后）与 L479（if 校验）之间

> **同进程 harness 设计**：提取 Step 4 代码，在 L478 与 L479 之间注入：
> ```powershell
> # T38-stats-items 注入：篡改 stats 使完整性校验失败
> $doc.stats.total = 999
> ```
> 注入点在 foreach 循环后、if 校验前，技术干净。
...
执行是否落入 L480 try 块                          — 检查 stdout 中是否出现 REVIEW_WRITE_OK| 或其他异常
tmp 是否被清理                                    — result.review.tmp 状态
```

> **判定规则**：如果 L479 路径输出 `RUN_STATUS|failed|` 后落入 L480 try 块并再次输出 `RUN_STATUS|failed|`（因 tmp 写入可能成功但后续校验会因 stats/items 不一致而失败），则违反 constraint #13"仅输出一次"。此情况判定为 **T38-stats-items FAIL + P1**。
>
> 如果 L479 路径输出 `RUN_STATUS|failed|` 后落入 L480 但后续因 `$doc.review` 已设置而正常完成 review 写入（输出 `REVIEW_WRITE_OK|`），则违反"不可恢复错误后不应继续执行"的语义。此情况同样判定为 **FAIL**。
>
> 如果 L479 路径行为正确（如落入 L480 后因某种原因安全终止），须在 test-report.md 中详细说明实际行为链。
...
- [ ] T38-stats-items: 核验是否落入 L480 try 块（检查后续输出）
...
**检查方法**: 运行 Step 1 + Step 2 后，检查 T46 测试目录（非生产根目录）是否出现同名状态文件。如果测试目录根出现 → FAIL + P1。
...
### Phase 7: T04 + T26 + T18 — 状态机 + Flag + 状态保留（中）
...
#### 7.2 T26 — Strict Flag Regression（Prompt §15）
...
#### 7.3 T18 — State Preservation Regression（Prompt §16）
...
验证 `Get-ResponseHeaderValue`（SKILL L324-328）在 `System.Net.WebHeaderCollection` 上的行为。
...
**额外检查**：搜索所有 `RUN_STATUS|failed|` 输出点，确认是否有可能**重复输出**（constraint #13 要求"仅输出一次"）。特别关注 L479 stats/items 完整性失败路径（return 被移除，执行可能落入后续 try 块导致二次输出）。
...
3. 入库：commit 文件范围显式列出——
   ...
   - `.exec-plan/exec-plan-v1.10-a.md`（本执行计划）
...
| §13 T04 rate_limited 回归 | Phase 7 | T04-PS7 |
...
| 编号 | 决策 | 理由 |
|---|---|---|
| D1 | 新增 T38-heartbeat / T38-result-read / T38-stats-items 子测试 | Prompt §5 要求"不要只测试 Set-Content"；Prompt §19 要求搜索所有 return；L476 heartbeat 和 L477 result.json-read 是 v1.10 新增路径，必须独立验证 |
| D2 | L479 stats/items 路径 return 移除发现 | v1.9→v1.10 diff 显示 L479 的 `return` 被移除，执行落入 L480 try 块。此不一致性是本轮关键发现，T38-stats-items 子测试专门验证此行为 |
...
| D6 | T38 子测试从 3 个扩展到 6 个 | Prompt §6 要求 T38-A/B/C；但 Prompt §19 要求搜索所有 return；L476 heartbeat / L477 result-read / L479 stats-items 是 v1.10 新增或修改路径，必须独立验证 |

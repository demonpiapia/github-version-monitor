...
2. 禁止复用 `.production-validation/`、`.production-validation-v17-final/`、`.production-validation-v18-final/`、`.production-validation-v19-final/` 中的实际测试状态作为当前 PASS 证据（Prompt §1: `OLD_EVIDENCE_USED_AS_CURRENT_PASS = NO`）
...
> **PS5.1 语法兼容要求**：`mock-invoke-restmethod.ps1` 和 `step2-mock-harness.ps1` 在 Phase 1（PS7 环境）创建，但被 Phase 8（PS5.1 环境）复用。这两个脚本须使用 PS5.1 兼容语法（禁用 PS7-only 运算符如 `??`、`? :`（三元）、`&&`/`||`（管道链）等），否则 Phase 8 意外失败的原因与被测对象无关。Phase 1 对齐自检中须增加语法兼容检查。
...
3. 禁止复用 .production-validation/ / .production-validation-v17-final/ / .production-validation-v18-final/ / .production-validation-v19-final/ 中的旧产物作为本轮 PASS 证据
...
- [x] 枚举/数量词全部来自完整原文阅读（28 项能力 / 9 项禁止 / 9 种状态 / 6 场景 / 7 个 invariant）
...
### exec-plan-v1.10-d（2026-09-09，三轮审计修订版）

**审计来源**: `.exec-plan/exec-plan-v1.10-c-codebuddy-review.md`（CodeBuddy 三轮独立审计）

**审计结论**: PASS（0 P1 + 0 P2 + 3 P3，均不阻断执行）

| 审计编号 | 严重性 | 发现 | 采纳/驳回 | 修订内容 | 理由 |
|---|---|---|---|---|---|
| P3-C1 | P3 | 禁止项计数"10 项"与清单实际 9 项不符 | 采纳 | 3 处"10 项"改为"9 项"（Phase 1 审查点 + §8 A1 + 附录 A §4 映射表） | 独立核实：Phase 1 Step 3 清单逐项点数为 9 项（mock URL / forced success / debug bypass / test-only branch / hardcoded token / hardcoded test repository / skip schema / skip lock / skip commit）。审计报告提到 2 处，独立核实发现第 3 处（附录 A §4 映射表），一并修正 |
| P3-C2 | P3 | Clean-Room 禁复用清单遗漏 `.production-validation-v19-final/` | 采纳 | Phase 0 第 2 条 + §3 第 3 条补入 `.production-validation-v19-final/` | 独立核实：Test-Path 确认该目录存在=True（v17/v18 亦存在，`.production-validation/` 不存在）。Prompt §1 列 4 个禁复用目录，c 版两处均仅列 3 个。Phase 11 self-review 第 3 项已提及 v19，与 Phase 0/§3 内部不一致，现已统一 |
| P3-C3 | P3 | PS5.1 兼容示例"`<# #>` 内联"事实错误 | 采纳 | 删除 `<# #>` 内联示例，改为 `? :`（三元）、`&&`/`||`（管道链） | 独立核实：`<# #>` 是 PowerShell 块注释语法，自 PS 2.0 起即支持，并非 PS7-only。`??`（null 合并）、`? :`（三元）、`&&`/`||`（管道链）确属 PS7-only |
...
| §4 Diff Integrity | Phase 1 | v19-v110.diff + 28 项能力核验 + 9 项禁止项 + 6 路径核验 |

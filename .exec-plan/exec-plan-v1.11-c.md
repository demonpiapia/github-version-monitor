...
> **版本**: exec-plan-v1.11-c（基于 exec-plan-v1.11-b 修订，采纳/处置 Zoo agent 审计报告 `exec-plan-v1.11-b-zoo-review.md`，逐项处置记录见 §9 修订日志）
> **制定日期**: 2026-09-09
...
**进程模型维度区分**（v1.11-b 修订补充）：上述锁 PID 一致性机制仅解决 **ownership 校验维度**（heartbeat/锁释放的 PID 匹配）。**故障注入维度**（文件锁持有、目录监视）物理上必须使用**独立后台进程**——同一进程对同一文件持有 `FileShare::None` 句柄后无法再次打开该文件（自身冲突抛 `IOException`，SKILL L477 heartbeat 即此模式）。因此 `lock-holder.ps1` / `watch-dir.ps1` 一律经 `Start-Process -WindowStyle Hidden` 启动为独立进程，测试后 `Stop-Process` 终止；编排器自身进程不得持有任何与被测路径冲突的句柄。
...
| 12 | Self-Review | {{ISO8601}} | {{ISO8601}} | {{status}} | 15 Prompt + 4 模板 = 19 项复核完成 | {{result}} |
...
|---|---|---|---|
| T38-A | L487 RUN_STATUS 行 / L488 return 行（catch 内） | **不出现** | tmp 写入失败 → return |
| T38-B | L496 RUN_STATUS 行 / L497 return 行（catch 内） | **不出现** | JSON 校验失败 → return |
| T38-C | L506 RUN_STATUS 行 / L507 return 行（catch 内） | **不出现** | 原子替换失败 → return |
...
| T23 | 无 return（L629 catch 后落入锁释放与终态输出） | **出现** | 替换失败 → 清理 → 锁释放 → RUN_STATUS\|failed\|（L653）→ 自然结束 |
...
- [ ] 确认 9 项辅助禁止项检查完成（计划内部防御性检查，非 Prompt 明文要求，见 Phase 1 Step 4 与附录 B D9）
...
- 预创建 `result.review.tmp`，由**独立后台进程**（`lock-holder.ps1`，`Start-Process -WindowStyle Hidden` 启动）以 `[System.IO.File]::Open()` 独占锁定（`FileShare::None`）——同一进程无法对同一文件持锁后再次写入，物理上必须独立进程——阻止 `Set-Content` 覆盖
...
重点覆盖 Step 1-5 **全部** return（**不能只检查本次修复的 L480**，Prompt §26 明确）。v1.11-c 基线 = 完整枚举实测（`Select-String '\breturn\b'` 共 42 行命中：39 处代码 return + 3 处正文文本 L81/L768/L769 排除），逐项归类如下（执行时仍须重新枚举复核）：

| 位置 | 类型 | terminal status |
|---|---|---|
| Step 1 L172（STATE_MISSING） | 正常控制流 | `STATE_MISSING\|`（非 RUN_STATUS 契约路径） |
| Step 1 L222（LOCKED） | 正常控制流 | `LOCKED\|` |
| Step 2 L254（锁不存在） | fatal | `RUNTIME_ERROR\|`（L253）— **审计点：此路径缺 RUN_STATUS 终态**（按 SKILL 实际 contract 如实判定并记录） |
| Step 2 L266/L269/L277/L280/L294/L296/L299/L310/L313/L326/L329 | 函数 return | Step 2 内部函数定义（Release-LockSafely/版本解析/版本比较/时间转换/Get-ResponseHeaderValue）内的 return，非整轮终止路径 |
| Step 2 L320（PARSE_ERROR） | fatal | `PARSE_ERROR\|` + 锁释放尝试 — **审计点：此路径缺 RUN_STATUS 终态**（同上） |
| Step 2 L406（fetch tmp 校验失败） | fatal | `RUNTIME_ERROR\|`（L404）— **审计点：此路径缺 RUN_STATUS 终态**（同上） |
| Step 2 L412（result.json 替换失败） | fatal | `RUNTIME_ERROR\|`（L410）— 同上 |
| Step 3 L444 catch[IOException] | 正常控制流 | `LOCKED\|`（L444 行内，保守退出） |
| Step 3 L444 catch 通用（heartbeat 失败） | fatal | `RUNTIME_ERROR\|` — **审计点：此路径缺 RUN_STATUS 终态**（同上） |
| Step 4 L476（Release-LockSafely 函数定义） | 函数 return | Step 4 本地函数内 return，非整轮终止路径 |
| Step 4 L477（heartbeat） | fatal（单行 catch） | `RUNTIME_ERROR\|` + `RUN_STATUS\|failed\|` ×1（输出与 return 同行） |
| Step 4 L478（result-read） | fatal（单行 catch） | `RUNTIME_ERROR\|` + `RUN_STATUS\|failed\|` ×1（同行） |
| Step 4 L480（stats/items，**v1.11 修复**） | fatal（单行 if 内） | `REVIEW_WRITE_ERROR\|` + `RUN_STATUS\|failed\|` ×1（同行）+ return |
| Step 4 L488（tmp 写入失败 return 行） | fatal | `REVIEW_WRITE_ERROR\|`（L485）+ `RUN_STATUS\|failed\|`（L487）×1 + return（L488） |
| Step 4 L497（JSON 校验失败 return 行） | fatal | `REVIEW_WRITE_ERROR\|`（L493）+ `RUN_STATUS\|failed\|`（L496）×1 + return（L497） |
| Step 4 L507（原子替换失败 return 行） | fatal | `REVIEW_WRITE_ERROR\|`（L503）+ `RUN_STATUS\|failed\|`（L506）×1 + return（L507） |
| Step 5 L529（锁不存在） | fatal | `RUNTIME_ERROR\|`（L529 同行）— **审计点：此路径缺 RUN_STATUS 终态**（同上） |
| Step 5 L534（heartbeat IOException） | 正常控制流 | `LOCKED\|`（L534，保守退出；Step 5 heartbeat 无通用 catch） |
| Step 5 L607（md tmp 失败 return 行） | fatal | `RUN_STATUS\|failed\|主 md 未提交。`（L606）×1 + return（L607） |
| Step 5 尾段（无 return，三分支终态） | — | L651 success / L649 + L653 failed |

> 上表为 v1.11-c 完整枚举基线（v1.11-b 初扫框架经 Zoo 审计 P2-001/P2-002 指出遗漏后，按 `Select-String '\breturn\b'` 实测补全）；Phase 11 执行时仍须重新枚举复核，每个 fatal return 的 "exactly once" 结论须引用行号 + 对应输出语句。fatal 审计点路径（L254/L320/L404/L410/L444 通用 catch/L529）按 SKILL 实际 contract 判定并记录，不隐瞒不扩大（D6）。
...
> **报告 O 节（Execution Summary）必须包含 PS5.1 双维度计数裁决声明**：production-critical（PS7）vs compatibility（PS5.1）为**计划 §6.3 自设裁决**（消解 Prompt §20"PS5.1 不得阻塞 PS7 production gate"与 §35"FAIL>0 → PRODUCTION_NOT_READY"的字面冲突），非 Prompt 原文要求，报告中须如实注明（codexCLI 审计 P2-004 采纳）。
...
- [x] 枚举/数量词来自完整原文阅读（30 项能力 = Prompt §4 清单 / 9 种状态 = T18 非 ok 状态（Prompt §22 清单，SKILL queryStatus 共 10 值含 ok）/ 9 项 T26 输入 = Prompt §21 / 6 场景 = Prompt §17 / 6 子测试 = Prompt §5-§12 / 16 章报告 = Prompt §32）；15+4=19 项 self-review 为**计划复合设计**（Prompt §31 固定 15 项 + 模板 §6.3 补充 4 项）；9 项辅助禁止为**计划内部防御性检查**（继承 v1.10 轮实践，非 Prompt 要求，见附录 B D9）

### A2 模块化与串行
- [x] 模块总览表完整（14 个主 Phase，Phase 0-13；Phase 13 含 6 个子任务 13.1-13.6）；每模块输入封闭、输出清单化
- [x] 模块间仅通过产物文件传递状态
...
- [x] 独立复核模块检查集完整（Phase 12 含 15 Prompt + 4 模板 = 19 项，含指纹复核、受限 diff、守恒核算）
...
### exec-plan-v1.11-c（2026-09-09，Zoo agent 审计修订版）

**审计来源**: `.exec-plan/exec-plan-v1.11-b-zoo-review.md`（Zoo agent 独立审计：0 P1 + 2 P2 + 3 P3，diff 核验/注入设计/SENTINEL 逻辑/判定规则/Prompt 覆盖全部通过）

| 审计项 | 严重性 | 采纳/驳回 | 修订内容 | 理由（[实测] 前缀标注依据类型） |
|---|---|---|---|---|
| P2-001 | P2 | **采纳（超额）** | Phase 11.1 框架表补充审计指出的 2 处：Step 2 L254（锁不存在）/ Step 5 L529（锁不存在）fatal return；另按完整枚举实测补充审计未列出的：L320（PARSE_ERROR fatal 审计点）/ L534（Step 5 heartbeat IOException→LOCKED 正常控制流）/ Step 2 内部函数 return 区（L266/L269/L277/L280/L294/L296/L299/L310/L313/L326/L329）/ Step 4 L476（Release-LockSafely 函数 return）；框架表升级为完整枚举基线（42 行命中：39 处代码 return + 3 处正文文本 L81/L768/L769 排除） | [实测] `Select-String '\breturn\b'` 完整枚举 42 行；`运行锁不存在` 实测 L253/L529；L252-254 与 L529 结构与审计描述逐字一致。审计仅指出 2 处，独立枚举发现框架表共遗漏 8 类 return，一并补齐（v1.10-d C1 修订同款超额采纳惯例）；框架表由"初扫框架"升级为"完整枚举基线"，消除执行时依赖执行者自行枚举的不确定性 |
| P2-002 | P2 | **采纳** | 框架表 Step 3 L444 拆分为两行：catch[IOException]（`LOCKED\|`，正常控制流）/ catch 通用（`RUNTIME_ERROR\|`，fatal 审计点） | [实测] L444 原文确含两个 catch 子句各自 return（`LOCKED\|运行锁无法独占` 实测命中 L444）；原框架表仅列通用 catch，遗漏 IOException 分支归类 |
| P3-001 | P3 | **采纳** | SENTINEL 表 T38-A/B/C 行号修正为双行号标注：L487 RUN_STATUS 行/L488 return 行、L496/L497、L506/L507 | [实测] L488/L497/L507 = return 语句行（逐行提取确认），L487/L496/L506 = RUN_STATUS 输出行；原表将输出行标为"return 位置"，off-by-one 属实。采纳"标注两个行号"建议——保留 RUN_STATUS 行号，因其为 §12 统一计数的验证锚点 |
| P3-002 | P3 | **采纳** | SENTINEL 表 T23 行"L630 catch"→"L629 catch" | [实测] L629 = `} catch {`、L630 = catch 内首句 `Remove-Item $tmp...` |
| P3-003 | P3 | **采纳** | 框架表 Step 4 三处同步改为双行号（与 SENTINEL 表一致），并补充对应 `REVIEW_WRITE_ERROR\|` 输出行号（L485/L493/L503） | [实测] 与 P3-001 同源；REVIEW_WRITE_ERROR 完整枚举 L470/L480/L485/L493/L503/L518（L470/L518 为正文文字，非代码输出） |

**驳回项**: 无（5 项发现全部经独立实测确认成立并采纳）

**未采纳/冲突项**: 无。审计 §11 R-001（Step 2/3 的 RUNTIME_ERROR 路径缺 RUN_STATUS 终态属 SKILL 设计而非计划缺陷）与计划 D6/Phase 11.1 审计点设计一致，确认维持；审计对 SKILL-v1.11 L252-254/L529 锁检查代码的引用经逐字核对无误。

---

### exec-plan-v1.11-b（2026-09-09，codexCLI 审计修订版）

**审计来源**: `.exec-plan/exec-plan-v1.11-a-codexCLI-review.md`（Codex CLI 独立审计 + 主 agent 二次复核：推翻 1 项 P3、修正引用节号 1 项）

**审计结论**: 需修订后执行（2 P1 + 4 P2 + 2 P3；所有问题均为表述/口径层面，行号/注入设计/判定规则核验通过）

| 审计项 | 严重性 | 采纳/驳回 | 修订内容 | 理由（[实测]/[原文]/[规范] 前缀标注依据类型） |
|---|---|---|---|---|
| P1-001 | P1 | **采纳** | 4 处"19 项"补构成标注：§0.9 追踪表 Phase 12 行 / §8 A4 / 附录 A §31 行 / 附录 C（L1470/L1480/L1860 原已标注，未动） | [实测] Select-String 确认"19 项"共 10 处，其中 4 处无构成标注，易误导为 Prompt 要求 19 项；[原文] Prompt §31 仅固定 15 项。注：审计引用位置"L244（总览表）"实测无"19 项"字样，实际未标注处为 L203/L1799/L1902/L1935——实质问题成立，位置引用有误 |
| P1-002 | P1 | **采纳** | §8 A1 重构："9 项辅助禁止"移出"完整原文阅读"声明，单独标注"计划内部防御性检查（非 Prompt 要求）"；Phase 1 审查点同步补标注 | [实测] L1781 原文确将"9 项辅助禁止"列入"全部来自完整原文阅读"，与附录 B D9 自述"v1.11 Prompt §4 无该清单"构成自相矛盾；[原文] Prompt §4 仅 30 项能力清单，无禁止项清单 |
| P2-001 | P2 | **采纳** | §0.5 新增"进程模型维度区分"段（锁 PID 一致性仅解决 ownership 校验维度；故障注入须独立后台进程）；T38-A 文件锁构造补"独立后台进程"显式字样 | [原文] SKILL L477 `[IO.File]::Open(...,FileShare::None)` 为独占锁，同进程二次打开同一文件抛 IOException；[实测] T38-heartbeat/C/T23 原文已写"外部进程（lock-holder.ps1）"，T38-A 仅括号暗示、§0.5 未区分两个维度，按审计建议明确化 |
| P2-002 | P2 | **部分采纳** | 附录 C 与 §8 A2 统一为"14 个主 Phase（Phase 0-13），Phase 13 含 6 个子任务" | 采纳"口径统一"建议；**驳回**"Phase 13 拆分为 13.1-13.5 五个子阶段""实质 18 个执行单元"——[实测] 计划 Phase 13 为 13.1-13.6 **六个**子节（审计自报"5 个"与原文不符），"18 个执行单元"算法未定义、无可核实口径 |
| P2-003 | P2 | **采纳** | §0.2 基线"L487 tmp-write RUN_STATUS"→"L481 Set-Content tmp（tmp 写入动作）/ L487 tmp 写入失败 RUN_STATUS（失败终态输出）" | [实测] L481=`$doc\|ConvertTo-Json\|Set-Content -Path $tmpPath`（写入动作）、L487=`Write-Output 'RUN_STATUS\|failed\|review 写入失败，整轮终止。'`（终态输出），原表述易误读 |
| P2-004 | P2 | **采纳** | Phase 13.1 新增要求：报告 O 节须包含 PS5.1 双维度计数裁决声明（计划自设裁决，非 Prompt 原文） | [原文] Prompt §20"PS5.1 不得阻塞 PS7 production gate"与 §35"FAIL>0 → PRODUCTION_NOT_READY"确系字面冲突；裁决已在 §6.3/附录 B D8 声明，本次补报告层显式记录 |
| P3-001 | P3 | **驳回** | 无修订 | [规范] Markdown（GFM）表格单元格内 `|` 必须转义为 `\|`——即使处于反引号 code span 内（GFM 表格解析优先于 code span 的管道豁免，反引号方案反而断表）；当前计划表格渲染正常。另注：审计报告 §4 核验表 B 项自身行号口径不一致（"Hunk2 L477 / Hunk3 L765"为 diff hunk 头行号，实际变更行为 L480/L768），其引用精度不足以支撑此项修订 |
| P3-003 | P3 | **部分采纳** | §8 A1 对"9 种状态"补出处标注："T18 非 ok 状态（Prompt §22 清单），SKILL queryStatus 共 10 值含 ok" | **驳回**"来源模糊/删除 9 种"主张——[原文] Prompt §22 完整列出 9 种非 ok 状态清单（not_found/rate_limited/server_error/network_error/invalid_response/metadata_incomplete/auth_error/forbidden/http_error），"9 种"出处明确且计划 Phase 8.3 已完整枚举；审计声称"Prompt 仅在标题写 9 种"与原文不符（§22 标题为"state preservation regression"无"9 种"字样，且审计对该项的节号引用 §21/§22 自相混）；[实测] SKILL queryStatus 10 值 = 9 非 ok + ok，与"9 种"不矛盾。采纳补标注以消除歧义 |
| 审计 §5.1（主 agent 推翻 P3-002） | — | **确认维持** | 无修订 | [原文] Prompt 章节标题实测：§16=T39 / §17=T43 / §18=T46 / §19=T04，计划附录 A 映射正确；codex 的 P3-002 系审计员误读，主 agent 推翻正确 |
| 审计 §6 未核验项 4（git 追踪状态） | — | **已补核** | 无修订（附录 B D10 声明成立） | [实测] `git check-ignore` 对 `.exec-plan/` 与 `.GPT/` 两文件均 exit=1（未被忽略），`git status` 显示 SKILL-v1.11.md、.GPT prompt、.exec-plan 两个文件均 untracked；Phase 0 git add / Phase 13 commit 设计无障碍 |
| 审计 §6 未核验项 1-3 | — | **不适用计划修订** | 无修订 | ACL 可写性（T22）/独立进程锁可行性（T38-A/C）/PS5.1 存在性均为执行期验证项；计划 Phase 3/2/9 已有 BLOCKED 兜底与失败处理路径；PS5.1 于本机已实证存在（5.1.22621.963，2026-09-09 实测记录），但执行机若不同仍按 Phase 9 兜底处理 |

**驳回项**: P3-001（GFM 表格内 `\|` 为必需转义，反引号方案反而破坏表格解析）；P2-002 的部分内容（"5 个子阶段""18 个执行单元"与计划原文不符且无口径）

**未采纳/冲突项**: 无（除上述驳回理由外，全部意见经独立核实后处置；审计报告自身另发现 2 处内部不一致——核验表 B 项行号口径、P1-001 位置引用——已在对应行注明，不影响其结论有效性）
...
| §31 Self-Review | Phase 12 | 15 Prompt 项 + 带时间戳文件名（另 +4 模板补充项） |
...
- Phase 数：14 个主 Phase（Phase 0-13，Phase 13 含 6 个子任务；9+ 符合 L 型）
- §0.6 独立性原则：保留全文
- §0.4 断点机制：完整 progress.json + 恢复原则
- 模块骨架：标准三段（Phase 0 基线 / Phase 12 复核 / Phase 13 收尾）+ 硬门槛分级（Phase 2/3/4/5/7）
- Phase 12 独立复核：完整检查集（15 Prompt + 4 模板 = 19 项）

...
4. [六层工作流分层 The 6-Layer Workflow](#4-六层工作流分层-the-6-layer-workflow)
   ...
   - [L6 `.runtime/`](#l6-runtime--运行时状态层-runtime-state)
5. [Handoff 文件标准字段](#5-handoff-文件标准字段-handoff-schema)
6. [待议层：`.contract/`](#6-待议层contract--deferred-layer)
7. [Git 仓库结构（推荐）](#7-git-仓库结构推荐-recommended)
   - [`.Template/`](#template--可复用模板目录-reusable-templates)
8. [全景流程图](#8-全景流程图-workflow-map)
9. [模板落地清单](#9-模板落地清单-adoption-checklist)
...
- 独立审计的对象是**仓库中的实际产物**（`.exec-plan/` / `.handoff/` / `.selfreview/` 等），不是聊天中的描述。
...
> 以下 6 层为**流程层**（执行链路）。
> 仓库内另有**非流程资产目录** `.Template/` 与 `.Teamwork-Guideline/`，不参与执行数据流，定义见 [§7](#7-git-仓库结构推荐-recommended)。
...
| `.Template/` | 可复用模板存放目录（非流程层，见下节） | 进 Git |
| `.Teamwork-Guideline/` | 工程约定与协作体系 spec 文件目录 | 进 Git |

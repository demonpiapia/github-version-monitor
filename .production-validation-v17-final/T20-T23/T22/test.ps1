...
<#
    T22 - md temp write failure
    Test: when the temp write + atomic move for GitHub更新监测列表.md fails
          (destination locked with FileShare::None), the main md is NOT corrupted.
    ...
    itself succeeds (md.tmp is a new file), but Move-Item md.tmp -> md fails.
    This is the temp-write + atomic-replace failure path.
#>
...
# ---- Fixture setup ---------------------------------------------------------
# Use a raw file write to control encoding/line-endings exactly.
# Note: here-string @"..."@ produces CRLF line endings and no BOM.
$origMd = @"
# GitHub 更新监测列表
...
（占位）
"@
# Write with UTF8NoBOM to avoid Set-Content's BOM insertion on PS5.1
...
# Build a modified md (new content) — the SKILL would produce this from result.json
$newMd = @"
# GitHub 更新监测列表

> 最近核对时间：2026-09-07 12:01（北京时间，本轮 1 项：latest API 成功 1 / 失败 0 / 待核 0；失败项保留上轮状态；数据来源 GitHub REST API 直连，无 HTML 回退）
...
# 1) Write to md.tmp (this should succeed — new file)
$tmpWriteError = $null
...
# 3) Attempt Move-Item md.tmp -> main md (this should FAIL because main md is locked)
$moveError = $null
$moveSucceeded = $false
$skillOutput = ''
if ($tmpWriteSucceeded -and $validatePassed) {
    try {
        ...
        # SKILL's catch block would NOT exist for Move-Item — but the surrounding
        # try/catch in production would catch this. For test purposes, we emit
        # the closest canonical message and clean up the temp file.
        $skillOutput = ('RUNTIME_ERROR|md 原子替换失败：{0}' -f $moveError)
        # Cleanup temp file (SKILL catch-block cleanup equivalent)
        Remove-Item $tmpPath -Force -ErrorAction SilentlyContinue
    }
} elseif ($tmpWriteSucceeded -and (-not $validatePassed)) {
    Remove-Item $tmpPath -Force -ErrorAction SilentlyContinue
    $skillOutput = 'VALIDATE_ERROR|临时文件结构校验未过，不替换主 md。'
} else {
...
# Verify main md content is unchanged (not partial write)
...
# Verify no partial content was written (e.g., contains '12:01' would mean partial write)
$containsNewTimestamp = $mdContentAfter -match '12:01'
Log "Main md does NOT contain new timestamp '12:01' (no partial write): $(-not $containsNewTimestamp)"

$tmpExists = Test-Path $tmpPath
Log "md.tmp still exists: $tmpExists (expected: False)"

# ---- Verdict --------------------------------------------------------------
$pass = $shaUnchanged -and $mdUnchanged -and (-not $containsNewTimestamp) -and (-not $tmpExists) -and (-not $moveSucceeded) -and ($tmpWriteSucceeded)
...
$report = @"
# Test Report: T22 - md temp write failure
...
Test that when the temp-write + atomic-move sequence for GitHub更新监测列表.md fails
(because the main md is locked with FileShare::None), the main md is NOT corrupted
and no partial write occurs. This validates SKILL-v1.7.md Step 5 (lines 560-583):
...
- No partial write (no '12:01' timestamp in main md): $noPartialStr
"@

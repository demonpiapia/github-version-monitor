# step4-t38b-harness.ps1 - T38-B 同进程 harness
# 目标：SKILL L490-491 `Get-Content $tmpPath -Raw|ConvertFrom-Json` 校验失败。
#
# 注入点：SKILL L489（第一个 try/catch 块结束 `}`）之后、L490（第二个 try 块开始 `try { $check=Get-Content $tmpPath...`）之前。
# 此切分点在两个 try/catch 块之间，技术干净。
#
# 注入内容：将 $tmpPath 内容替换为非法 JSON，使后续 ConvertFrom-Json 失败。
#
# 设计原因：pwsh -File 模式下脚本不可暂停；dot-source step4.ps1 会一次性执行完整 Step 4，
# 无法在 Set-Content 与 Get-Content 之间注入文件篡改。因此采用内联复制 + 精确注入方式。

# T04-PS7: 403 + X-RateLimit-Remaining=0 => queryStatus=rate_limited
# Method A: 从 step2.ps1 逐字提取 Get-ResponseHeaderValue / ConvertTo-UtcIso / 状态机 try/catch
# 通过 mock-listener.ps1 返回 403 + X-RateLimit-Remaining=0
$ErrorActionPreference = 'Stop'
$here = $PSScriptRoot
$lib = Join-Path (Split-Path $here -Parent) 'lib'
$step2 = Join-Path $lib 'step2.ps1'

# 记录 PowerShell 版本
$PSVersion = $PSVersionTable.PSVersion.ToString()
Write-Output "PS_VERSION|$PSVersion"

# 从 step2.ps1 逐字提取的函数（L85-91 Get-ResponseHeaderValue）
...
# 不设 Authorization：模拟无 token 场景；测试目标是 403 rate_limited，与 token 无关

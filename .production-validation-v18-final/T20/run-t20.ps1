# T20: result.json 原子性 — Move-Item result.fetch.tmp -> result.json 失败时，旧 result.json 不被损坏
$ErrorActionPreference = 'Continue'
$base = 'D:\AI\Workspace\automatic\github-version-monitor\.production-validation-v18-final\T20'
...
# Phase 1: run step1 + step2 in child to generate result.json
...
# Phase 2: lock result.json via lock-holder process, then run step2 to trigger Move-Item failure
$lockHolderScript = @'
...
# Phase 3: run step1 (creates fresh lock) + step2 in child (step2 will fail Move-Item)
$phase2Out = & pwsh -NoProfile -Command "`$env:GITHUB_VERSION_MONITOR_BASE='$base'; & '$lib\step1.ps1'; & '$lib\step2.ps1'" 2>&1
...
$tmpResult = Join-Path $base '.monitor\result.fetch.tmp'
$lockPath = Join-Path $base '.monitor\run.lock'

$stdoutContent = Get-Content (Join-Path $base 'stdout.txt') -Raw
$hasRuntimeError = $stdoutContent -match 'RUNTIME_ERROR\|result\.json 原子替换失败'

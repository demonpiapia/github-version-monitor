...
`RUN_STATUS|success|` / `RUN_STATUS|failed|` 是整轮唯一最终终态；`COMMIT_OK|` 仅为中间信号。
...
$doc.review=[PSCustomObject]@{performed=$true;items=@($reviewItems)};$newStats=$doc.stats|ConvertTo-Json -Depth 8 -Compress;$newItems=$doc.items|ConvertTo-Json -Depth 8 -Compress;if($newStats-ne $origStats-or $newItems-ne $origItems){Remove-Item $tmpPath -Force -ErrorAction SilentlyContinue;Write-Output 'REVIEW_WRITE_ERROR|review 修改了 stats/items，拒绝覆盖 result.json。';Release-LockSafely;return};$doc|ConvertTo-Json -Depth 8|Set-Content -Path $tmpPath -Encoding UTF8;try{$check=Get-Content $tmpPath -Raw|ConvertFrom-Json}catch{$check=$null};if($null-eq $check-or $null-eq $check.review-or ($check.stats|ConvertTo-Json -Depth 8 -Compress)-ne $origStats-or ($check.items|ConvertTo-Json -Depth 8 -Compress)-ne $origItems){Remove-Item $tmpPath -Force -ErrorAction SilentlyContinue;Write-Output 'REVIEW_WRITE_ERROR|review 临时 JSON 校验失败，不替换 result.json。';Release-LockSafely;return};try{Move-Item $tmpPath $resultPath -Force}catch{Remove-Item $tmpPath -Force -ErrorAction SilentlyContinue;Write-Output ('REVIEW_WRITE_ERROR|review 原子替换失败：{0}'-f $_.Exception.Message);Release-LockSafely;return};Write-Output ('REVIEW_WRITE_OK|复核完成：{0} 项；stats/items 保持不变。'-f $reviewItems.Count)
```
